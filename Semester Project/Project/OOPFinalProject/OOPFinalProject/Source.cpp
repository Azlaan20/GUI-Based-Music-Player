...
public:
	...
	void ChangePower()
	{
		...
		if (Power == false)
		{
			cout << "PRESS ENTER TO POWER ON" << endl;
			cin.get(in);
		...
		{
			cout << "PRESS THE ESC KEY TO POWER OFF" << endl;
			cin >> in;
	...
	void SelectMedia()
	{
		if (Power)
		{
			cout << "Do you want to insert a CD or a USB? CD = 1, USB = 2" << endl;
			cin >> MediaType;
			while (MediaType != 1 && MediaType != 2) //Validation
			{
	...
	bool PlayAudio()
	{
		...
		if (PLFinish == false)
		{
			...
			PlayCount++;
			cout << "ENTER 1 TO STOP THE SONG" << endl;
			cin >> input;
			if (input == 1)
			{
				StopSong();
...
public:
	void CreatePlayList()
	{
		...
		cout << "ENTER THE NAME OF YOUR PLAYLIST" << endl;
		cin >> PlayListName;
	...
	{
		cout << "PLAYLIST NAMES" << endl;
		for (int i = 0; i < List.size(); i++)
		{
			cout << i + 1 << ". " << List[i][0] << endl; //will display the name of each playlist
		}
	}
	...
	void AddSong()
	{
		...
		cout << "Please choose the playlist to which you would like to add a song" << endl;
		cin >> PLChoice;
